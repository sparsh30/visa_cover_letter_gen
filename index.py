...
st.title('🛂 Generate Cover Letters for any Visa!')
st.write('Use this application to generate a cover letter')
...
# Create a dropdown
selected_name = st.selectbox('Choose a Visa:', list(options.values()))
...
if start_date and pax_name and end_date:
    if start_date <= end_date:
        updated_letter = template.replace('[Start Date]', str(start_date.strftime('%B %d, %Y')))
        updated_letter = updated_letter.replace('[End Date]', str(end_date.strftime('%B %d, %Y')))
        updated_letter = updated_letter.replace('[PAX Name]', str(pax_name))

        # Display the dynamically generated letter
        st.write(f"**Generated Letter:**\n\n")

        updated_letter_code = f'''{updated_letter}'''
        st.code(updated_letter_code, language="text")

    else:
        st.write("ERROR: End Date is before Start Date")
else:
